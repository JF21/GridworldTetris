        ...
        PseudoInfiniteViewport.Pannable
{
    ...
    private static final int DEFAULT_CELL_SIZE = 58;
    ...
    private boolean checkerBoardOn=false;
    private Location colorizeLocation=null;
    private boolean colorize=false;
    private Color locColor=Color.WHITE;
    ...
    public void paintComponent(Graphics g)
    {
		...
		drawSolidBackground(g2, insets, Color.BLACK);
        drawWatermark(g2);
        drawGridlines(g2);
        if(checkerBoardOn)
       	 	drawCheckerBoard(g2);
        ...
        if(colorize)
        {
			...
			g2.fillRect((colorizeLocation.getCol()+1)*(cellSize+1)-cellSize ,(colorizeLocation.getRow()+1)*(cellSize+1) -cellSize,
										(cellSize + 1)-1 ,(cellSize + 1) -1);
		}
	...
	public void colorizeLocation(Location loc)
	{
		if(colorize)
		{
			colorize=false;
	...
	public void setCheckBoard()
	{
		...
		else checkerBoardOn=true;
	}


	public void drawSolidBackground(Graphics2D g2, Insets insets, Color color)
	{
		g2.setColor(color);
		g2.fillRect(insets.left, insets.top, numCols * (cellSize + 1) + 1, numRows* (cellSize + 1) + 1);
	}

	public void drawCheckerBoard(Graphics2D g2)
	{
		//This will set a checkerboard pattern for the background
		int colorCount=0;
			...
			for(int i=1;i<=numCols;i++)
			{
				...
				{
					//alternate color pattern for checkerboard
						if(colorCount%2==0)
							g2.setColor(Color.BLACK);
						else g2.setColor(Color.RED);
					 g2.fillRect(i*(cellSize+1)-cellSize ,j*(cellSize+1 ) -cellSize, (cellSize + 1)-1 ,(cellSize + 1) -1);
					 colorCount++;
				}
				//Check for dimensions to see if we need to swap colors or not...
				//Used to handle odd or even or mismatched dimension sizes
				if((numCols%2==1 && numRows%2==0) || (numCols%2==0 && numRows%2==0))
					colorCount++;
			}
	}
    ...
    private void drawGridlines(Graphics2D g2)
    {
       ...
       g2.drawLine(305,0,305,595);

       g2.drawLine(455,446,455,575);//y
       g2.drawLine(325,446,325,575);//y
       g2.drawLine(325,446,455,446);//x
       g2.drawLine(325,576,455,576);//x

       g2.drawLine(455,0,455,425);
	   g2.drawLine(325,0,325,425);
	   g2.drawLine(325,425,455,425);
	   g2.drawLine(325,125,455,125);
